...
def convert_idiom(apkg_path, output_path):
    ...
    for note in notes:
        ...
        if len(fields) >= 2:
            ...
            if idiom and definition:
                words_list.append({"word": idiom, "definition": definition})
    ...
    if words_list:
        print(
            f"  示例: {words_list[0]['word']} - {words_list[0]['definition'][:30]}..."
        )
    return words_list
...
def convert_classical(apkg_path, output_path):
    ...
    if words_list:
        print(
            f"  示例: 第{words_list[0]['sentence_num']}句 - {words_list[0]['content'][:30]}..."
        )
    return words_list
...
def convert_japanese_n1(apkg_path, output_path):
    ...
    if words_list:
        sample = words_list[0]
        print(
            f"  示例: {sample['word']} ({sample['kanji']}) - {sample['definition'][:30]}..."
        )
    return words_list
...
if __name__ == "__main__":
    ...
    # 转换各词库
    convert_idiom(
        new_dir / "_reversed_card.apkg", languages_dir / "idiom" / "words.json"
    )

    convert_classical(new_dir / "_.apkg", languages_dir / "classical" / "words.json")

    convert_radio(new_dir / "A.apkg", languages_dir / "radio" / "words.json")

    convert_japanese_n1(
        new_dir / "N1.apkg", languages_dir / "japanese_n1" / "words.json"
    )
